...
export class ProductCustomDataPackageAComponent implements OnInit {
  ...
  getData() {
    console.log("hehe");
    this.fileService.get("business-codes.json").subscribe((res) => {
      this.businessCodes = res;
      console.log(this.businessCodes);
    });

    this.fileService.get("sector.json").subscribe((res) => {
      this.sectors = res;
      console.log(this.sectors);
    });

    this.fileService.get("division.json").subscribe((res) => {
      this.divisions = res;
      console.log(this.divisions);
    });

    this.fileService.get("company-status.json").subscribe((res) => {
      this.companyStatus = res;
      console.log(this.companyStatus);
    });

    this.fileService.get("state-codes.json").subscribe((res) => {
      this.stateCodes = res;
      console.log(this.stateCodes);
    });

    this.fileService.get("company-origins.json").subscribe((res) => {
      this.companyOrigins = res;
      console.log(this.companyOrigins);
    });

    this.fileService.get("company-types.json").subscribe((res) => {
  ...
  search() {
    ...
    this.productService.getList(this.customForm.value).subscribe(
      ...
      () => {
        this.updateTable();
        // to reduce the quota after searching process
        this.quotaService.update(this.quota[0].id).subscribe((res) => {
          this.checkQuota();
        });
      }
    );
  ...
  checkQuota() {
    this.quotaService
      .filter(
        "user=" + this.cookieService.getCookie("userId") + "&quota_type=0A"
      )
      .subscribe(
        (res) => {
          // console.log("res", res);
          this.quota = res;
          this.calculateQuota = this.quota[0].quota;
        },
        (err) => {
  ...
  confirm() {
    ...
    this.customForm.controls["incorpDtFrom"].setValue(
      this.customForm.value.incorpDtFrom + "T00:00:00"
    );
    this.customForm.controls["incorpDtTo"].setValue(
      this.customForm.value.incorpDtTo + "T00:00:00"
    );

    this.bizCodes.forEach((biz) => {
      if (cnt == 0) {
  ...
  noQuota() {
    Swal.fire({
      ...
      if (res) {
        // Confirm
        this.cartService
          .addItem(this.cartService.cartCurrent.id, this.quotaForm.value)
          .subscribe(() => {
            let title = "Success";
  ...
  addToCart(row) {
    ...
    this.criteriaService.create(body).subscribe(
      ...
      () => {
        this.cartService
          .addItem(this.cartService.cartCurrent.id, this.cartForm.value)
          .subscribe(() => {
            let title = "Success";
            let message = "Item is added to the cart";
            this.toastr.success(message, title);
