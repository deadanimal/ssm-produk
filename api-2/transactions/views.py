...
class TransactionViewSet(NestedViewSetMixin, viewsets.ModelViewSet):
    ...
    def pg_return(self, request, *args, **kwargs):
        ...
        if transaction_status == '0':
            ...
            data_loaded['transaction'].total_amount = format(
                total_amount_original/100, '.2f')

            html_string = render_to_string(
                'receipt/receipt.html', {'data': data_loaded})
            html = HTML(string=html_string)
            pdf_file = html.write_pdf(stylesheets=[CSS(css_file)])
            # pdf_file = html.write_pdf()

            file_path = "ssm/receipt/ssm-receipt" + "-" + \
                datetime.datetime.utcnow().strftime("%s") + '.pdf'
    ...
    def latest_successful(self, request, *args, **kwargs):
        ...
        serializer = TransactionExtendedSerializer(
            all_latest_successful_transactions, many=True)
        return Response(serializer.data)
    ...
    def report(self, request, *args, **kwargs):
        ...
        all_ok_transactions = Transaction.objects.filter(
            payment_status='OK').filter()
        serializer = TransactionExtendedSerializer(
            all_ok_transactions, many=True)
        return Response(serializer.data)
    ...
    def generate_report_table(self, request, *args, **kwargs):
        ...
        if report_column == 'csv':
            ...
            for transaction_row in filtered_table:
                ...
                transaction_ = Transaction.objects.filter(
                    id=transaction_row.id).first()
                cart_items_ = CartItem.objects.filter(
                    cart=transaction_.cart).all()
                for cart_item_row in cart_items_:
                    if cart_item_row.product and cart_item_row.entity:
                        ...
                        product_ = Product.objects.filter(
                            id=cart_item_row.product.id).first()
                        entity_ = Entity.objects.filter(
                            id=cart_item_row.entity.id).first()

                        if entity_.type_of_entity == 'AD':
                            entity_reg_no_ = entity_.audit_firm_number
                        elif entity_.type_of_entity == 'BS':
                            entity_reg_no_ = entity_.registration_number_new + \
                                '(' + entity_.registration_number + \
                                '-' + entity_.check_digit + ')'
                        elif entity_.type_of_entity == 'CP':
                            entity_reg_no_ = entity_.company_number_new + \
                                '(' + entity_.company_number + \
                                '-' + entity_.check_digit + ')'

                        new_row = [
                            transaction_.created_date.astimezone(
                                pytz.timezone(time_zone)).strftime('%d-%m-%Y %-H:%M:%S'),
                            transaction_.name,
    ...
    def generate_gaf(self, request, *args, **kwargs):
        ...
        start_date_new_ = make_aware(datetime.datetime.strptime(
            start_date_, '%Y-%m-%dT%H:%M:%S.%fZ'))
        end_date_new_ = make_aware(datetime.datetime.strptime(
            end_date_, '%Y-%m-%dT%H:%M:%S.%fZ'))
        ...
        combined_data_.append([
            ...
            '',
            '000640557056',
            start_date_new_.astimezone(
                pytz.timezone(time_zone)).strftime('%d/%m/%Y'),
            end_date_new_.astimezone(pytz.timezone(
        ...
        for transaction_row in filtered_table:
            ...
            for supply_item in cart_items_:
                if supply_item.product and supply_item.entity:
                    ...
                    product_ = Product.objects.filter(
                        id=supply_item.product.id).first()
        ...
        for transaction_ledger in filtered_table:
            transaction_ = Transaction.objects.filter(
                id=transaction_ledger.id).first()
            cart_items_ = CartItem.objects.filter(cart=transaction_.cart).all()
            item_counter = 1
            ...
            for ledger_item in cart_items_:
                if ledger_item.product and ledger_item.entity:
                    ...
                    product_ = Product.objects.filter(
                        id=ledger_item.product.id).first()
                    ...
                    print('Sum: ', credit_sum, 'Transaction ID',
                          transaction_.receipt_no)
